...
<div class="page-header">
    ...
    <a class="btn btn-primary pull-right ajax-load" href="<?=Route::url('oc-panel',array('controller'=>'forum','action'=>'create'))?>">
  <?=__('New forum')?></a>
</div>
...
<?function lili($item, $key,$forums){?>
    <li data-id="<?=$key?>" id="li_<?=$key?>"><i class="icon-move"></i> <?=$forums[$key]['name']?>
        ...
        <a 
            ...
            data-id="li_<?=$key?>" 
            data-text="<?=__('We will move the siblings forums and ads to the parent of this forum.')?>" 
